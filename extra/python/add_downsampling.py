...
def add_downscale(group_path: Path, new_scale: int, downscaling):
    ...
    if len(dsf) != new_scale:
        raise RuntimeError(
            f"Expected {new_scale} existing elements in downsamplingFactors, got {len(dsf)}"
        )
    ...
    try:
        ...
        logger.info(
            f"Group has no resolution information, not writing to dataset s{new_scale} metadata"
        )
        return
    ...
    if "units" in ds_attrs or "resolution" in ds_attrs:
        logger.info(
            f"Resolution info already found in dataset s{new_scale}, not overwriting"
        )
        return
...
def main(args=None):
    ...
    parser.add_argument(
        "group", type=group_name, help="Qualified name of multiscale N5 group"
    )
    parser.add_argument("newscale", type=int, help="The scale level of the new array")
    parser.add_argument(
        "downsampling",
        type=parse_downsampling,
        help="Single-step downsampling as comma-separated integers",
    )
